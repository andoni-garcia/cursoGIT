                ...
                <#list orderingOptions as item>
                    ...
                    <#if ( sortField?has_content && sortField == item )>
                        <#if ( sortOrder?has_content && sortOrder == "ASC" )>
                            <option value="${orderingOptionUrlDESC}">▼ <@fmt.message key="ci.order.${item}"/></option>
                            <option selected value="${orderingOptionUrlASC}">▲ <@fmt.message key="catalogue_list.order.${item}"/></option>
                        <#else>
                            <option selected value="${orderingOptionUrlDESC}">▼ <@fmt.message key="catalogue_list.order.${item}"/></option>
                            <option value="${orderingOptionUrlASC}">▲ <@fmt.message key="catalogue_list.order.${item}"/></option>
                        </#if>
                    <#else>
                        <option value="${orderingOptionUrlDESC}">▼ <@fmt.message key="catalogue_list.order.${item}"/></option>
                        <option value="${orderingOptionUrlASC}">▲ <@fmt.message key="catalogue_list.order.${item}"/></option>
                    </#if>
                </#list>
            </select>
