...
<@fmt.message key="eshop.captchaError" var="errorValidatingCaptcha"/>

<@fmt.message key="eshop.accept" var="labelAccept"/>
<@fmt.message key="eshop.cancel" var="labelCancel"/>

<script src="https://www.google.com/recaptcha/api.js?lang=${lang}" async defer></script>
...
<script>
    function onSubmit(token) {
        ...
        if(response.length !== 0){
            $("#registerFormBtn").removeClass("disabled");
...
<main class="smc-main-container eshop" style="margin-bottom: 10rem">
    ...
    <form class="container" action="${actionURL}&${_csrf.parameterName}=${_csrf.token}" method="post" id="registerForm">
        ...
        <div class="row">
            ...
            <div class="col-12 mt-5">
                <button type="submit" id ="registerFormBtn" class="btn btn-primary disabled" disabled = "disabled" ><@fmt.message key="users.registerBtn"/></button>
            </div>
        </div>
    </form>
