var SelSeries = Class
		.extend({
			init : function($) {
				...
				this.selection.valve._default = this.valve_size.SY1000;
...
//						this.onBotonClicked);
			...
			getConjunto : function() {
				...
				var m = this.getSelectedManyfold() || { name : 'IP40' };
				var p = this.getSelectedPiping() || { name : 'SIDE' };
			...
			activateConnectionField : function() {
				var isIp67 = this.getSelectedManyfold().name === 'IP67' ? true : false;
				var blank;
				...
				for ( var idx in allConnections){
					if (typeof allConnections[idx] !== 'function'){
						...
						if(this.jQueryCache.connections.filter(allConnections[idx]).id = 'c_blank') {
							blank = this.jQueryCache.connections.filter(allConnections[idx]);
						}
						//console.log(this.jQueryCache.connections.filter(allConnections[idx]));
					}
				}
